...
def add_menu(user_name,body_data):
    url = '/api/menus/'
    headers = {'Content-Type': 'application/json','Authorization': 'Bearer '+SkiGlobalData().get_global_data('jwt_access')[user_name['name']] }
    login_user=body_data['detail']
    r = d_requests.post(url=BASE_URL+url,headers=headers,json=login_user)    # 最基本的Post请求
    logger.info("response is : {0}".format(r.text))
    return r
...
def update_menu(user_name,sid,body_data):
    ...
    headers = {'Content-Type': 'application/json','Authorization': 'Bearer '+SkiGlobalData().get_global_data('jwt_access')[user_name['name']] }
    login_user=body_data['detail']
    r = d_requests.put(url=BASE_URL+url,headers=headers,json=login_user)    # 最基本的Put请求
    logger.info("response is : {0}".format(r.text))
    return r

def delete_menu(user_name,sid):
    url = '/api/menus/{0}/'.format(str(sid))
    headers = {'Content-Type': 'application/json','Authorization': 'Bearer '+SkiGlobalData().get_global_data('jwt_access')[user_name['name']] }
    r = d_requests.delete(url=BASE_URL+url,headers=headers)    # 最基本的delete请求
    logger.info("response is : {0}".format(r.text))
    return r
